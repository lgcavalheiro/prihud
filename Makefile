...
dev-docker-run: dev-check-image
	docker run --rm -v ${PWD}/${DB}:${APP_DIR}/${DB}:delegated -p 127.0.0.1:${PORT}:${PORT} --name ${PROJECT} ${PROJECT}_dev:latest
...
prod-docker-run: prod-check-image
	docker run --rm -v ${PWD}/${DB}:${APP_DIR}/${DB}:delegated -p 127.0.0.1:${PORT}:${PORT} --name ${PROJECT} ${PROJECT}_prod:latest
